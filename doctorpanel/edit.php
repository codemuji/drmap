...
// Check if doctor has edit permission
$canEdit = isset($doctor['can_edit']) ? (int)$doctor['can_edit'] : 1;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Check edit permission before processing
    if (!$canEdit) {
        $error = 'Your profile editing access has been restricted by the administrator. Please contact support for assistance.';
    } else {
        ...
        if (empty($error)) {
            try {
                ...
                $params = [
                    ...
                    // Normalize addresses: store as JSON array or NULL
                    'addresses' => (function($raw){
                        if ($raw === null || $raw === '') return null;
                        // If already valid JSON (array or object), keep as JSON string
                        $dec = json_decode($raw, true);
                        if (json_last_error() === JSON_ERROR_NONE && is_array($dec)) {
                            return json_encode($dec);
                        }
                        // Otherwise, split on newlines and store as JSON array
                ...
                $stmt->execute($params);
                 ...
                 // Update doctor-hospital associations (Req 16)
...
<div class="flex min-h-screen">
    ...
    <aside class="w-[280px] bg-gradient-to-b from-slate-900 to-slate-800 text-white fixed h-full hidden lg:flex flex-col z-50">
        ...
        <nav class="flex-1 px-3 space-y-1 overflow-y-auto py-6">
            ...
            <a href="edit.php" class="flex items-center px-4 py-3 rounded-xl font-medium text-white bg-primary-600">
    ...
    <main class="flex-1 lg:ml-[280px]">
        ...
        <header class="glass sticky top-0 z-40 border-b border-white/50">
            <div class="px-6 lg:px-8 py-4">
                <div class="flex items-center justify-between">
                    <div class="flex items-center space-x-4">
                        ...
                        <div>
                            <h1 class="text-xl font-bold text-slate-900">Edit Profile</h1>
                        </div>
                    </div>
                    <a href="view.php" class="px-4 py-2.5 text-primary-600 border-2 border-primary-600 rounded-xl text-sm font-medium hover:bg-primary-50">
            ...
            <form method="POST" enctype="multipart/form-data" class="space-y-6" <?php echo !$canEdit ? 'onsubmit="return false;"' : ''; ?>>
                <!-- Personal Information Section -->
                <div class="card-premium rounded-2xl p-6 lg:p-8 <?php echo !$canEdit ? 'opacity-60 pointer-events-none' : ''; ?>">
                ...
                <div class="card-premium rounded-2xl p-6 lg:p-8 <?php echo !$canEdit ? 'opacity-60 pointer-events-none' : ''; ?>">
                    ...
                    <div class="space-y-5">
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-slate-900 mb-2">Clinic Addresses</label>
                            <textarea name="addresses" rows="4" class="w-full px-4 py-2.5 border-2 border-slate-200 rounded-xl input-premium"
                                      placeholder="Enter each address on a new line&#10;Example:&#10;City Hospital, 123 Main Street, New York, NY 10001&#10;Heart Care Clinic, 456 Park Avenue, New York, NY 10022"><?php
                                // If addresses stored as JSON array, show as newline-separated for editing
                                $addrRaw = $doctor['addresses'] ?? '';
                                $addrDec = json_decode($addrRaw, true);
                                if (json_last_error() === JSON_ERROR_NONE && is_array($addrDec)) {
                                    echo htmlspecialchars(implode("\n", $addrDec));
                                } else {
                                    echo htmlspecialchars($addrRaw);
                                }
                            ?></textarea>
                            <p class="text-xs text-slate-500 mt-2">Enter each clinic/hospital address on a separate line</p>
                        </div>
                        ...
                        <div>
                             ...
                             <input type="text" name="practice_city" id="practice_city" value="<?php echo htmlspecialchars($doctor['practice_city'] ?? ''); ?>"
                                    class="w-full px-4 py-2.5 border-2 border-slate-200 rounded-xl input-premium" placeholder="e.g., New York">
                             <p class="text-xs text-slate-500 mt-2">City where you are currently practicing (used for search and filters)</p>
                         </div>

                         <!-- Hospital / Clinic Associations (Req 16) -->
                         <div>
                             ...
                             <p class="text-xs text-slate-500 mt-2">Check the hospitals/clinics where you practice</p>
                         </div>
                ...
                <div class="card-premium rounded-2xl p-6 lg:p-8 <?php echo !$canEdit ? 'opacity-60 pointer-events-none' : ''; ?>">
                    ...
                    <div class="space-y-6">
                        ...
                        <!-- Locations editor (Disabled for doctor - Req 32) -->
                        <div class="p-4 bg-slate-50 border border-slate-200 rounded-xl">
                            <label class="block text-sm font-semibold text-slate-500 mb-1">Locations & GPS Pins</label>
                            <p class="text-xs text-slate-500">Google location GPS pins and clinic addresses are handled exclusively by the administrator team and cannot be modified from the doctor portal.</p>
                            <textarea name="locations" id="hf_locations" class="hidden"><?php echo htmlspecialchars($doctor['locations'] ?? ''); ?></textarea>
                        </div>
                ...
                <div class="bg-blue-50 border border-blue-200 rounded-2xl p-6">
                    <div class="flex gap-4">
                        ...
                        <div>
                            <h4 class="font-semibold text-blue-900 mb-1">Tip</h4>
                            <p class="text-sm text-blue-800">
                                All changes you make here will be immediately reflected on your profile page and the doctors list. 
                ...
                <div class="flex gap-3 justify-end sticky bottom-0 bg-white/95 backdrop-blur-lg -mx-6 lg:-mx-8 px-6 lg:px-8 py-4 border-t border-slate-200">
                    <a href="view.php" class="px-6 py-2.5 border-2 border-slate-300 rounded-xl font-semibold text-slate-900 hover:bg-slate-50 transition-colors">
                        <?php echo $canEdit ? 'Cancel' : 'Back to Profile'; ?>
                    </a>
                    <?php if ($canEdit): ?>
...
<div id="mapPickerModal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
    <div class="bg-white rounded-2xl w-full max-w-4xl h-[70vh] overflow-hidden flex flex-col">
        <div class="flex items-center justify-between p-3 border-b sticky top-0 bg-white" style="z-index:1200">
            <h3 class="font-semibold">Pick location on map</h3>
            <div class="flex items-center gap-2">
                <button id="mapPickerCancel" class="px-3 py-1 rounded bg-slate-100">Cancel</button>
                <button id="mapPickerConfirm" class="px-3 py-1 rounded bg-primary-600 text-white">Use this location</button>
            </div>
        </div>
        <div id="mapPickerContainer" class="flex-1"></div>
    </div>
</div>
...
function initMapPicker(){
    ...
    // initialize map
    mapPicker = L.map(container).setView([20,0], 2);
    ...
    // If Cancel/Confirm handlers were not bound earlier, bind/override here for Leaflet-specific behavior
    const cancel = document.getElementById('mapPickerCancel');
    const confirm = document.getElementById('mapPickerConfirm');
    ...
    confirm?.addEventListener('click', ()=>{
        ...
        if (mapPickerMarker) {
            ...
            mapPickerTargetLat.value = latlng.lat;
            mapPickerTargetLng.value = latlng.lng;
            mapPickerTargetLat.dispatchEvent(new Event('input', { bubbles: true }));
            mapPickerTargetLng.dispatchEvent(new Event('input', { bubbles: true }));
        } else {
            const center = mapPicker.getCenter();
            mapPickerTargetLat.value = center.lat;
            mapPickerTargetLng.value = center.lng;
            mapPickerTargetLat.dispatchEvent(new Event('input', { bubbles: true }));
            mapPickerTargetLng.dispatchEvent(new Event('input', { bubbles: true }));
        }
...
// Open picker and bind target inputs
function openMapPicker(latInput, lngInput){
    const modal = document.getElementById('mapPickerModal');
    mapPickerTargetLat = latInput; mapPickerTargetLng = lngInput;
    // show modal
    modal.classList.remove('hidden');
    // Wait a tick for map container to size
    setTimeout(()=>{
...
document.addEventListener('DOMContentLoaded', ()=>{
    ...
    confirmBtn?.addEventListener('click', ()=>{
        if (!mapPickerTargetLat || !mapPickerTargetLng) { modal.classList.add('hidden'); return; }
        if (mapPickerMarker && typeof mapPickerMarker.getLatLng === 'function') {
            const latlng = mapPickerMarker.getLatLng();
            mapPickerTargetLat.value = latlng.lat;
            mapPickerTargetLng.value = latlng.lng;
            mapPickerTargetLat.dispatchEvent(new Event('input', { bubbles: true }));
        ...
        // If leaflet not ready, try to use map center if available
        try {
            if (mapPicker && typeof mapPicker.getCenter === 'function') {
                const c = mapPicker.getCenter();
                mapPickerTargetLat.value = c.lat; mapPickerTargetLng.value = c.lng;
                mapPickerTargetLat.dispatchEvent(new Event('input', { bubbles: true }));
                mapPickerTargetLng.dispatchEvent(new Event('input', { bubbles: true }));
                modal.classList.add('hidden');
                return;
...
// Parse initial JSON from hidden fields
function parseJSONSafe(str) {
    if (!str && str !== '') return null;
    // If a DOM node was passed accidentally
    ...
    // Helper to decode common HTML entities (e.g., &quot;)
    ...
    // Try progressive parsing: raw -> decode entities -> unwrap double-encoded JSON
    try {
        let v = JSON.parse(str);
        // If parse returns a string, it may be double-encoded: try parse again
        if (typeof v === 'string') {
            ...
            try {
                ...
                // try parsing v directly
                try { return JSON.parse(v); } catch (e3) { return v; }
            }
        }
        return v;
    } catch (e) {
        // Try decode HTML entities then parse
        try {
...
function renderLocations(){
    ...
    const data = parseJSONSafe(raw) || [];
    data.forEach(l => addLocationRow(l.address||'', l.lat||'', l.lng||'', l.mapEmbedUrl||''));
}

function addLocationRow(address='', lat='', lng='', map=''){
    const row = createEl(`
        <div class="editor-row border p-3 rounded">
            <div class="flex gap-2 mb-2">
                <input type="text" class="flex-1 px-3 py-2 border rounded loc-address" placeholder="Address" value="${address.replace(/\"/g,'&quot;')}">
                <input type="text" class="w-32 px-3 py-2 border rounded loc-lat" placeholder="Lat" value="${lat}">
                <input type="text" class="w-32 px-3 py-2 border rounded loc-lng" placeholder="Lng" value="${lng}">
            </div>
            <div class="flex gap-2 items-center">
                <input type="text" class="flex-1 px-3 py-2 border rounded loc-mapembed" placeholder="Map embed URL (optional)" value="${map.replace(/\"/g,'&quot;')}">
                <button type="button" class="px-3 py-1 text-sm rounded bg-amber-100 text-amber-800 pick-map-btn">Pick on map</button>
                <button type="button" class="px-2 py-1 text-sm text-red-600 border rounded remove-btn">Remove</button>
            </div>
        </div>
    `);
    ...
    bindRemove(row.querySelector('.remove-btn'));
    // wire pick on map
    const pickBtn = row.querySelector('.pick-map-btn');
...
function addVideoRow(url=''){
    const row = createEl(`
        <div class="editor-row flex gap-2 items-center">
            <input type="text" class="flex-1 px-3 py-2 border rounded" placeholder="Video URL or path (e.g. https://youtube.com/...)" value="${String(url).replace(/\"/g,'&quot;')}">
            <button type="button" class="px-2 py-1 text-sm text-red-600 border rounded remove-btn">Remove</button>
        </div>
    `);
    document.getElementById('videosEditor').appendChild(row);
    bindRemove(row.querySelector('.remove-btn'));
}
...
function resolveGallerySrc(p){
    ...
    // If path starts with '/', make it relative to app root
    const parts = location.pathname.split('/').filter(Boolean);
    const appRoot = parts.length ? '/' + parts[0] : '';
    if (s.startsWith('/')) return appRoot + s;
    // If path already relative like 'uploads/..', prefix with appRoot
    return appRoot + '/' + s;
}
...
function addImageRow(url=''){
    ...
    const row = createEl(`
        <div class="editor-row group bg-gradient-to-br from-slate-50 to-slate-100 rounded-xl p-3 border border-slate-200 hover:border-slate-300 hover:shadow-md transition-all">
            <div class="flex gap-3 items-center">
                ...
                <input type="text" class="flex-1 px-4 py-2.5 border border-slate-200 rounded-lg font-mono text-xs focus:ring-2 focus:ring-emerald-500 focus:border-transparent bg-white" placeholder="Image URL or path" value="${url.replace(/"/g,'&quot;')}">
    ...
    `);
    document.getElementById('galleryEditor').appendChild(row);
    bindRemove(row.querySelector('.remove-btn'));
}
...
// Profile photo upload handler (reuse upload_image.php)
...
photoUploadInput?.addEventListener('change', async function(){
    ...
    const origText = photoUploadBtn.textContent;
    ...
    try {
        ...
        try {
            ...
            console.error('Profile upload: invalid JSON response', text);
            alert('Upload error: invalid server response. See console for details.');
            throw new Error('Invalid JSON response');
        }

        if (!res.ok) {
            console.error('Profile upload failed', res.status, text, data);
            alert('Upload failed: ' + (data.error || ('HTTP ' + res.status)));
        } else if (data && data.success && data.url) {
            // set the photo URL input to uploaded URL
            const urlInput = document.getElementById('photoUrlInput');
            urlInput.value = data.url;
            // update preview using absolute_url when available, fallback to resolveGallerySrc
            try {
                const src = (data.absolute_url && String(data.absolute_url).trim()) ? data.absolute_url : resolveGallerySrc(data.url);
                const img = document.getElementById('photoPreviewImg');
                const icon = document.getElementById('photoPreviewIcon');
                if (img) { img.src = src; img.classList.remove('hidden'); }
                if (icon) icon.classList.add('hidden');
            } catch (e) { console.error(e); }
            // prevent duplicate upload on form submit
            try { photoUploadInput.removeAttribute('name'); photoUploadInput.value = ''; } catch (e) { console.error(e); }
        } else {
            console.error('Profile upload unexpected response', data, text);
    ...
    photoUploadBtn.textContent = origText;
    this.value = '';
});

// Update preview when URL input changes (also used on load)
...
function renderTiming(){
    ...
    days.forEach(d=>{
        ...
        const card = createEl(`
            <div class="day-card border border-slate-200 rounded-lg p-4" data-day="${d}">
                <div class="flex items-center justify-between mb-3">
        ...
        slots.forEach(slot=>{
            addRangeRow(rangesContainer, slot.open || '', slot.close || '');
        });
        ...
        toggle.addEventListener('change', ()=>{
            ...
            if (toggle.checked && slots.length === 0) {
...
function addReviewRow(name='', rating=5, comment='', date=''){
    ...
    `);
    document.getElementById('reviewsEditor').appendChild(row);
    bindRemove(row.querySelector('.remove-btn'));
}
...
document.querySelector('form').addEventListener('submit', function(e){
    // Quick facts
    const q = Array.from(document.querySelectorAll('#quickFactsEditor .editor-row')).map(r=>{
        const inputs = r.querySelectorAll('input');
        return { label: inputs[0].value.trim(), value: inputs[1].value.trim() };
    }).filter(x=>x.label || x.value);
    document.getElementById('hf_quick_facts').value = JSON.stringify(q);

    // Locations
    const locs = Array.from(document.querySelectorAll('#locationsEditor .editor-row')).map(r=>{
        const addr = (r.querySelector('.loc-address') || r.querySelector('input[placeholder="Address"]')).value.trim();
        const lat = (r.querySelector('.loc-lat') || r.querySelector('input[placeholder="Lat"]')).value.trim();
        const lng = (r.querySelector('.loc-lng') || r.querySelector('input[placeholder="Lng"]')).value.trim();
        const map = (r.querySelector('.loc-mapembed') || r.querySelector('input[placeholder="Map embed URL (optional)"]')).value.trim();
        const obj = { address: addr };
        if (lat!=='') obj.lat = parseFloat(lat);
        if (lng!=='') obj.lng = parseFloat(lng);
        if (map!=='') obj.mapEmbedUrl = map;
        return obj;
    }).filter(x=>x.address);
    document.getElementById('hf_locations').value = JSON.stringify(locs);

    // Timing (serialize week calendar with multiple ranges per day)
    const dayCards = Array.from(document.querySelectorAll('#timingWeek .day-card'));
    const timingObj = {};
...
document.addEventListener('DOMContentLoaded', ()=>{
    ...
    // initialize profile photo preview from current hidden input value
    try {
